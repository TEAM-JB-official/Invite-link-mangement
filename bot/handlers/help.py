...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_text = (
        ...
        "/addgroup <group_id> <title> – Manually add a single group/channel to the database.\n"
        "/addgroups <group_id:title> <group_id:title> ... – Add multiple groups at once.\n\n"
        "Admin Management\n"
        "/admins – List all admins and open management menu.\n"
        "/addadmin <user_id> <role> – Directly add an admin (role: admin or super_admin).\n\n"
        "Welcome Messages & Logs\n"
        "/settings – Interactive menu to set welcome message and log channel.\n"
        "/setwelcome <group_id> <message> – Set welcome message for a group.\n"
        "   Use {user} for user's first name, {link_creator} for creator ID.\n"
        "/setlogchannel <group_id> <channel_id> – Set log channel for a group.\n\n"
        "Link Management\n"
        "/revoke_all – Revoke all active invite links (global).\n"
        "/single <chat_id> [expiry_days] – Create a single‑use invite link instantly.\n\n"
        "Database Backup\n"
