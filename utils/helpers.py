...
async def create_invite_link(bot: Bot, group_id: int, creator_id: int, expiry_date: datetime, max_uses: int):
    db = get_db()
    # Create Telegram invite link with join request
    ...
    link_doc = {
        "link_id": link.invite_link.split("/")[-1],  # Telegram link ID
...
async def revoke_link_by_id(link_id: str, bot: Bot):
    ...
    if not link or link.get("is_revoked", False):
        return
    try:
        # Telegram does not have a direct revoke method, but we can edit the link to expire immediately
...
async def send_log(message: str):
    if LOG_CHANNEL:
        from bot.database.mongo import get_db
        db = get_db()
        # In real usage, you'd have a bot instance passed. Here we assume a global bot.
        # We'll handle via context in handlers. For simplicity, this function is called with a bot.
        pass  # Placeholder - actual logging is done inside handlers with context.bot

async def send_welcome(group_id, user, creator_id, bot):
    db = get_db()
